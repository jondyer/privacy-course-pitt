...
\begin{itemize}\tightlist
  \item policy1.xml -- A small, simple policy file for a single home computer for a family of 4
  \item policy2.xml -- A slightly more complicated file for a small business with several employees
  \item policy3.xml -- A larger policy file for several mid-sized entities utilizing more complex features
  \item policy4.xml -- A very large file (roughly 8,000 lines) presenting a complicated series of entities utilizing all of the features heretofore discussed
\end{itemize}
The files were tested by repeating 100 times the following procedure (on each) and then averaging the results:
\begin{enumerate}\tightlist
  \item Parse the entire file and record the time taken to complete preprocessing
  \item Query the engine with the standardized query ("Can Alice access timing.txt with write privilege?") and record time taken to receive response
\end{enumerate}
The results of 10 different repititions of this 100-iteration procedure are shown below both as a table and graphically. Note that the timing results for querying the enforcement engine were so small that I had to multiply by 100 in order for them to be visible on the chart (in other words, the engine is very fast once fully parsed!).
